...
- 提交前建议执行：`pnpm build`，并执行乱码扫描命令快速检查是否出现乱码字符（如 `�`）：`rg -n "�|鏇|鍒|缂|璐|鍝|鎿|绫诲瀷|鍦板潃|鐜" src`。

### 乱码防护约束（必做）
- 修改包含中文的 `*.vue`、`*.ts`、`*.md` 文件后，必须人工检查文件内容确认中文文案可读（可使用编辑器或 `Get-Content -Path <file> -Encoding utf8`），不允许出现“鏇/鍒/缂/璐”等异常字形。
...
## 多 CLI 并行协作约束
- 当前仓库允许多个 Code CLI 并行修改；若发现非本人产生的改动，默认视为正常协作状态。
- 非本人改动无需额外关注、无需阻塞当前任务，也不得擅自回退、覆盖或丢弃。
- 在保证不破坏现有改动的前提下，直接基于当前工作区状态继续完成自己的任务即可。
