...
describe('topology V3 API', () => {
  ...
  it('getTopologyTaskViewV3 should invoke get_topology_task_view_v3', async () => {
    ...
    const mockResult = {
      ...
      insights: [{ kind: 'dependency', title: 'summary', nodeIds: ['node-1'], edgeIds: [] }],
    }
  ...
  it('getTopologyPathsV3 should invoke get_topology_paths_v3', async () => {
    ...
    const mockResult = { paths: [{ nodeIds: ['A', 'B'], score: 1 }], truncated: false, totalCount: 1 }
  ...
  it('getTopologyEvidenceV3 should invoke get_topology_evidence_v3', async () => {
    ...
    const mockResult = {
      items: [{ id: 'ev-1', evidenceType: 'audit', title: 'recent update', nodeId: 'node-1' }],
      total: 1,
    }
