...
const SELECT_COLUMNS: &str =
    "id, name, category, type, address, port, version, \
     env, description, is_deleted, deleted_at, created_at, updated_at";

#[tauri::command]
pub fn list_middlewares(pool: State<DbPool>, params: QueryParams) -> Result<PagedResult<Middleware>, String> {
    ...
    let rows = stmt.query_map(param_refs.as_slice(), row_to_middleware)
...
#[tauri::command]
pub fn get_middleware(pool: State<DbPool>, id: String) -> Result<Middleware, String> {
    let conn = pool.get().map_err(|e| format!("Pool error: {}", e))?;
    let sql = format!(
    ...
    conn.query_row(&sql, rusqlite::params![id], row_to_middleware)
        .map_err(|e| format!("Middleware not found: {}", e))
}

#[tauri::command]
pub fn save_middleware(pool: State<DbPool>, data: Middleware) -> Result<(), String> {
    validate_middleware(&data)?;
    ...
    let result: Result<(), String> = (|| {
        if is_new {
            ...
            conn.execute(
                ...
                rusqlite::params![id, data.name, data.category, data.mw_type, data.address, data.port,
                                 data.version, data.env, data.description, now],
            ...
            conn.execute(
                ...
                rusqlite::params![data.name, data.category, data.mw_type, data.address, data.port,
                                 data.version, data.env, data.description, now, data.id],
            ).map_err(|e| format!("Update failed: {}", e))?;
            insert_audit_log(&conn, "update", "middleware", &data.id, Some(&data.name), None)?;
        }
        Ok(())
    })();
...
#[tauri::command]
pub fn soft_delete_middleware(pool: State<DbPool>, id: String) -> Result<(), String> {
    let conn = pool.get().map_err(|e| format!("Pool error: {}", e))?;
    ...
    match soft_delete(&conn, "middlewares", &id) {
        Ok(()) => {
            match insert_audit_log(&conn, "delete", "middleware", &id, name.as_deref(), None) {
                Ok(()) => { conn.execute_batch("COMMIT;").map_err(|e| e.to_string())?; Ok(()) }
                Err(e) => { let _ = conn.execute_batch("ROLLBACK;"); Err(e) }
            }
        }
