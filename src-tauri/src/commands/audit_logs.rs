...
#[tauri::command]
pub fn list_audit_logs(pool: State<DbPool>, params: QueryParams) -> Result<PagedResult<AuditLog>, String> {
    let conn = pool.get().map_err(|e| format!("Pool error: {}", e))?;

    // Build where clause manually (audit_logs has no is_deleted)
