...
#[tauri::command]
pub fn list_dependencies(pool: State<DbPool>, params: QueryParams) -> Result<PagedResult<Dependency>, String> {
    let conn = pool.get().map_err(|e| format!("Pool error: {}", e))?;

    let search_columns: &[&str] = &[];
    let filter_columns = &["source_id", "source_type", "target_id", "target_type", "relation_type"];
    ...
    let rows = stmt.query_map(param_refs.as_slice(), row_to_dependency)
...
#[tauri::command]
pub fn save_dependency(pool: State<DbPool>, data: Dependency) -> Result<(), String> {
    ...
    let result: Result<(), String> = (|| {
        if is_new {
            ...
            conn.execute(
                ...
                rusqlite::params![id, data.source_id, data.source_type, data.target_id, data.target_type,
                                 data.relation_type, data.description, now],
            ...
            conn.execute(
                ...
                rusqlite::params![data.source_id, data.source_type, data.target_id, data.target_type,
                                 data.relation_type, data.description, now, data.id],
...
#[tauri::command]
pub fn soft_delete_dependency(pool: State<DbPool>, id: String) -> Result<(), String> {
    let conn = pool.get().map_err(|e| format!("Pool error: {}", e))?;
    ...
    match soft_delete(&conn, "dependencies", &id) {
        Ok(()) => {
            match insert_audit_log(&conn, "delete", "dependency", &id, None, None) {
                Ok(()) => { conn.execute_batch("COMMIT;").map_err(|e| e.to_string())?; Ok(()) }
                Err(e) => { let _ = conn.execute_batch("ROLLBACK;"); Err(e) }
            }
        }
